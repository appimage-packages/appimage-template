  ...
  describe 'build_kf5' do
      ...
      if need == true
        frameworks.each do |framework|
          ...
          if framework == 'phonon' || framework == 'phonon-gstreamer'
            options = '-DCMAKE_INSTALL_PREFIX:PATH=/opt/usr  -DSYSCONF_INSTALL_DIR=/opt/etc -DBUILD_TESTING=OFF -DPHONON_BUILD_PHONON4QT5=ON -DPHONON_INSTALL_QT_EXTENSIONS_INTO_SYSTEM_QT=TRUE'
    ...
    describe 'build_project' do
        it 'Retrieves sources that need to be built from source' do
          ...
          expect(sources.run_build(name, buildsystem, options, path)).to be(0), " Expected 0 exit Status"
          expect(system("cd /in/#{name} && qmlimportscanner -rootPath .")).to be(0), " Expected 0 exit Status"
        end
    end
